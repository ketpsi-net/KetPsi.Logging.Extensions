...
{
    public class LogWarningHandlerTests
    {
        private static Mock<ILogger> CreateLogger(bool isEnabled)
        {
            var mock = new Mock<ILogger>();
            mock.Setup(l => l.IsEnabled(LogLevel.Warning)).Returns(isEnabled);
            return mock;
        }
        ...
        public void Constructor_WhenNotEnabled_SetsNullsAndZeroOffsets()
        {
            ...
            var handler = new LogWarningHandler(literalLength: 10, formattedCount: 3, logger, out bool isEnabled);
        ...
        public void Constructor_WhenEnabled_RentsArraysAndZerosOffsets()
        {
            ...
            var handler = new LogWarningHandler(literalLength: 20, formattedCount, logger, out bool isEnabled);
        ...
        {
            var logger = CreateLogger(isEnabled: true).Object;
            var handler = new LogWarningHandler(0, 0, logger, out _);
        ...
        {
            var logger = CreateLogger(isEnabled: true).Object;
            var handler = new LogWarningHandler(0, 2, logger, out _);
        ...
        {
            var logger = CreateLogger(isEnabled: true).Object;
            var handler = new LogWarningHandler(0, 3, logger, out _);
        ...
        {
            var logger = CreateLogger(isEnabled: true).Object;
            var handler = new LogWarningHandler(0, 4, logger, out _);
        ...
        public void AppendFormatted_WhenDisabled_DoesNotThrowButFieldsRemainNull()
        {
            ...
            var logger = CreateLogger(isEnabled: false).Object;
            var handler = new LogWarningHandler(0, 1, logger, out bool isEnabled);
